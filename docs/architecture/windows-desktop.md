...
Dock 按扣除侧栏后的真实空间适配。当前排列方向放不下各面板的最小尺寸，而另一方向能同时满足
宽高约束时，`fitTree` 只切换绘制方向，例如把窄窗口中的会话与终端改为上下排列。面板、分隔条和
拖拽命中共用这个结果，保存的方向与比例保持不变，扩大窗口会恢复原排列。两个方向都容纳不下时
保留按最小尺寸比例压缩的行为；这类极小空间无法同时保证所有面板的阅读宽高。
