...
export function ModelSelect({ ariaLabel, disabled, ...selection }: ModelSelection & { ariaLabel: string; disabled?: boolean }) {
	const { t } = useI18n();
	const settings = useApp((s) => s.settings);
	const menu = usePopover();
	const models = settings ? availableModels(settings) : [];
	const selected = models.find(({ model }) => model.id === selection.value);
	const ambiguous = selected && models.filter(({ model }) => model.name.trim().toLowerCase() === selected.model.name.trim().toLowerCase()).length > 1;
	const label = selected ? ambiguous ? `${selected.model.name} · ${selected.provider.name}` : selected.model.name : selection.value ? t("model.unavailable") : selection.inheritLabel;
	return <>
		<button type="button" aria-label={ariaLabel} aria-haspopup="menu" aria-expanded={menu.open} disabled={disabled}
			onClick={menu.toggle} data-ly-tip={selected ? `${selected.provider.name} · ${selected.model.name}` : selection.value || selection.inheritDetail}
