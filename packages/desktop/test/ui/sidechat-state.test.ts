...
test("late attach snapshot merges new stream events without deleting older questions or duplicating included events", async () => {
	...
	pending.resolve({ revision: 1, messages: [user("Old question")], running: true }); await attaching;
...
test("background answers update only their session and are immediately visible on returning", async () => {
	api(async (id) => ({ messages: [user(id)], running: false, revision: 0 }));
	...
	response.resolve({ messages: [user("a"), answer], running: false, revision: 2 }); await returning;
});
...
test("A to B to A cannot be overwritten by the first A read", async () => {
	...
	api((id) => id === "b" ? Promise.resolve({ messages: [], running: false }) : ++reads === 1 ? first.promise : last.promise);
	const oldA = useSide.getState().attach("a"); await useSide.getState().attach("b");
	const newA = useSide.getState().attach("a");
	last.resolve({ messages: [user("Current A")], running: false }); await newA;
	first.resolve({ messages: [user("Stale A")], running: true }); await oldA;
...
test("failed submission shows an error and releases the composer instead of leaving a pending run", async () => {
	api(async () => ({ messages: [], running: false }), async () => { throw new Error("IPC unavailable"); });
...
test("rewind drops discarded answers and tool cards before the replacement arrives", async () => {
	api(async () => ({ messages: [user("Original"), answer], running: false })); await useSide.getState().attach("a");
