...
test("the repair finds a package manager, which is the entire point", { skip: posix }, async (t) => {
	...
	const nodeAt = where("node");
	if (!nodeAt) {
		t.diagnostic("no node on this host's own PATH");
	} else if (!FALLBACK_DIRS.includes(dirname(nodeAt))) {
		t.diagnostic(`node lives in ${dirname(nodeAt)}, which is not in static fallback directories`);
	} else {
